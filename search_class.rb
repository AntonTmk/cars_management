...
class SearchClass
  ...
  def edit_history(searches)
    if searches.select { |v| v.to_s.casecmp(@request.car_hash(v[:requests_quantity], v[:total_quantity]).to_s).zero? }.length > 0
      (0..searches.length - 1).each do |v|
        unless !searches[v].to_s.casecmp(@request.car_hash(searches[v][:requests_quantity], searches[v][:total_quantity]).to_s).zero?
